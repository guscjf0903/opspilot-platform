...
| `scripts/` | 모드별 실행 스크립트. 루트 `*.sh`는 호환 wrapper, 하위 폴더가 canonical 진입점 |
...
| 모드 | 목적 | 주요 진입점 |
| --- | --- | --- |
| Local Dev | PostgreSQL, Redpanda, backend, frontend 로컬 개발 | `scripts/local-dev/` |
| Local kind | kind cluster, sample-app, Prometheus, OpenCost 검증 | `scripts/local-kind/` |
| Mode A | GitHub Pages 정적 포트폴리오와 fixture demo | `scripts/mode-a-pages/` |
| Mode B Lite | EC2+k3s 기반 저비용 live Kubernetes 데모 | `scripts/mode-b-lite/` |
| Mode B | EKS 기반 ephemeral lab | `infra/terraform/envs/aws-dev-foundation/`, `infra/terraform/envs/aws-dev-ephemeral/` |

기존 `scripts/*.sh` 명령은 계속 동작하지만 새 문서와 새 작업에서는 하위 폴더의 모드별
스크립트를 우선 사용합니다. 자세한 매핑은 [scripts/README.md](./scripts/README.md)를 참고합니다.
...
| Mode B | EKS | AWS managed Kubernetes 역량 증명 | foundation은 유지 가능, ephemeral lab은 사용 후 destroy |
| Mode C | MSK proof | Amazon MSK 연동 증명 | 데모 직전 생성 후 제거 |

Mode B EKS는 전략 B로 나눕니다. `aws-dev-foundation`에는 ECR과 GitHub Actions ECR push
Role을 두고, `aws-dev-ephemeral`에는 VPC, EKS, managed node group, optional ALB/RDS를
둘 예정입니다. 이렇게 하면 EKS 실습 환경을 destroy해도 이미지 저장소와 GitHub Actions Role을
유지할 수 있습니다.

현재 Mode B는 EKS 최소 클러스터 Terraform 코드까지 구현되어 있습니다. 실제 AWS apply,
node Ready 확인, Helm 앱 배포는 다음 실행 단계에서 검증합니다.
...
ECR push를 사용하려면 먼저 Terraform의 `aws-dev-foundation` 환경으로 ECR repository와
GitHub Actions ECR push Role을 준비하고, GitHub 저장소 secret에
`AWS_GITHUB_ACTIONS_ROLE_ARN`을 설정해야 합니다.
